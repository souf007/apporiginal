...
document.addEventListener('DOMContentLoaded', function() {
    // Function to load delivery notes
    function loadBl(page) {
        ...
        fetch(`ajax.php?action=loadbl&${params.toString()}`)
            .then(response => response.text())
            .then(data => {
                document.querySelector('.lx-table-bl').innerHTML = data;
            });
    }

    // Function to load price range
    function loadPriceRange() {
        fetch('ajax.php?action=loadpricerange&type=bl')
    ...
    // Event listeners
    document.getElementById('keyword').addEventListener('keyup', () => loadBl(1));
    document.querySelector('.lx-price-filter').addEventListener('click', () => loadBl(1));
    document.querySelector('select[name="nbrows"]').addEventListener('change', () => loadBl(1));
